...
Mat mat_alloc(size_t rows, size_t cols) {
  ...
  m.cols = cols;
  m.stride = cols;
  m.es = NN_MALLOC(sizeof(*m.es) * rows *
                   cols); // dereference to get the sizeof the block
  NN_ASSERT(m.es != NULL);
  return m;
}
...
Mat mat_row(Mat m, size_t row) {
  return (Mat){
      .rows = 1, .cols = m.cols, .stride = m.stride, .es = &MAT_AT(m, row, 0)};
}
...
// not allocating memory in the function, instead prealloc memory for all
// matrixes. Multiple into mat c mem cpy signature
void mat_dot(Mat dst, const Mat a, const Mat b) {
